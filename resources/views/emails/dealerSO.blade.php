    ...
    <table class='body'>
        ...
        <tr class='content'>
            <td colspan=2 style='padding-top:30px'>
                <span class='hi'>Dealer Account {{$dealerHistory->account}} / {{$dealer}} </span><br>

                <b>The dealer order number # {{$dealerHistory->order_num}}</b><br>
                <b>Notes # {{$dealerHistory->notes}}</b><br> @if ($dealerHistory->dealerShipAddress===null) @else
                <table width="600px" class='pd-10'>
                    ...
                    <tr>
                        <td style='text-align: left; text-transform:uppercase; font-weight:700'>
                            {{$dealerHistory->dealerShipAddress->company}}<br> {{$dealerHistory->dealerShipAddress->address}}
                            <br> {{$dealerHistory->dealerShipAddress->city}} {{$dealerHistory->dealerShipAddress->province}}
                            {{$dealerHistory->dealerShipAddress->postalcode}} @if($dealerHistory->dealerShipAddress->country=="CA")CANADA
                            @else USA @endif
                            <br>
                ...
                <table style='font-size:10px;' width="600px;" class='p530'>
                    ...
                    @foreach ($dealerHistory->dealerDetails as $item)
        ...
        <br>
        <tr>
            <td class='footer' colspan=2>170 Zenway Blvd. Unit 2, Woodbridge, ON., L4H 2Y7 CANADA | 905-850-3433<br> &copy; {{date("Y")}} Golden Leaf
                Automotive. All rights reserved.</td>
        </tr>
    </table>
